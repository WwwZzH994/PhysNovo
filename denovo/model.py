...
logger = logging.getLogger("PhysNovo")
    ...
    **kwargs : Dict
    ...
    def beam_search_decode(
        ...
        for i in range(1, self.max_length + 1):
            ...
            scores[~decoded, : i + 1, :] = decoder_out.to(scores.dtype)
    ...
    def greedy_decode(
        self, memories: torch.Tensor, mem_masks: torch.Tensor, precursors: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Greedy decoding of spectrum predictions.

        Parameters
        ----------
        memories : torch.Tensor of shape (n_spectra, seq_len, d_model)
            Encoder memory representations for each spectrum.
        mem_masks : torch.Tensor of shape (n_spectra, seq_len) or similar
            Attention masks corresponding to `memories`.
        precursors : torch.Tensor of size (n_spectra, 3)
            Measured precursor information for each spectrum (mass, charge, m/z).

        Notes
        -----
        - The decoder returns per-step raw logits (step_scores) and optionally other outputs.
        - `scores` is preallocated with the same dtype as `memories` (via .type_as(memories)).
        To avoid dtype mismatches, decoder logits are cast to `scores.dtype` before being
        written into the `scores` tensor.

        Returns
        -------
        probs : torch.Tensor of shape (n_spectra, max_length + 1, vocab_size + 1)
            Softmax-normalized probabilities for each amino acid token at every time step.
            The first step's predictions are stored at index 0.
        tokens : torch.Tensor of shape (n_spectra, max_length + 1)
            Greedy-decoded token indices (argmax over `probs`) for each spectrum and step.
        """
        # Preallocate storage for scores:
        # shape: (n_spectra, max_length + 1, vocab_size + 1)
        scores = torch.zeros(
            memories.shape[0], self.max_length + 1, self.decoder.vocab_size + 1
        ).type_as(memories)

        step_scores, _ = self.decoder(None, precursors, memories, mem_masks)
        scores[:, :1, :] = step_scores.to(scores.dtype)

        # Initialize greedy tokens from current scores (argmax across vocab dimension).
        tokens = torch.argmax(scores, axis=2)

        # Iteratively decode next tokens until all sequences predict the stop token
        # or we reach max_length. Loop index `i` corresponds to the number of filled positions.
        for i in range(2, self.max_length + 2):
            # Determine which sequences have already produced a stop token anywhere.
            decoded = (tokens == self.stop_token).any(axis=1)
            if decoded.all():
                break
            
            step_scores, _ = self.decoder(
                tokens[~decoded, : (i - 1)],
                precursors[~decoded, :],
                memories[~decoded, :, :],
                mem_masks[~decoded, :],
            )
            scores[~decoded, :i, :] = step_scores.to(scores.dtype)

            # Update greedy tokens from updated scores
            tokens = torch.argmax(scores, axis=2)

        # Return softmax probabilities and greedy token indices.
    ...
    def validation_step(
        ...
        self.log("valid_aa_precision", float(aa_precision), **log_args)
        self.log("valid_aa_recall", float(aa_recall), **log_args)
        self.log("valid_pep_recall", float(pep_recall), **log_args)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        metrics = {
            ...
            "valid_pep_recall": callback_metrics[
                "valid_pep_recall"
            ].detach(),
        }
