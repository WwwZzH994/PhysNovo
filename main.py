"""The command line entry point for PhysNovo."""
import datetime
import logging
import os
import sys
...
@click.option(
    ...
    required=True,
    default="denovo",
    help="\b\nThe mode in which to run PhysNovo:\n"
...
def main(
    ...
    if output is None:
        output = os.path.join(
            os.getcwd(),
            f"PhysNovo_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}",
        )
    else:
    ...
    logger.info("PhysNovo version %s", str(__version__))
    ...
    # Run PhysNovo in the specified mode.
    if mode == "denovo":
        logger.info("Predict peptide sequences with PhysNovo.")
        writer = f"{output}_denovo.txt"
        model_runner.predict(peak_path, model, config, writer)
    elif mode == "eval":
        logger.info("Evaluate a trained PhysNovo model.")
        writer = f"{output}_denovo.txt"
        model_runner.evaluate(peak_path, model, config, writer)
    elif mode == "train":
        logger.info("Train the PhysNovo model.")
